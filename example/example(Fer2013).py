...
model =  [
			Conv({'f_num':32, 'f_size':3, 'pad':0, 'stride':1},batchnorm=True),
			Conv({'f_num':32, 'f_size':3, 'pad':0, 'stride':1},batchnorm=True),
			Pool(2,2,2),
...
net = Net(model)
net.initialize(	shape=(1,48,48), 
								af=GELU, 
								opt=Adam, 
								rate=0.0003, 
								init_mode='xaiver', 
								dtype=np.float32)
path = './fer2013_example/'
net.load(path)
